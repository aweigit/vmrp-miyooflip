...
void getDestinationRect(int machine_width,int machine_height)
{
	double scale = min( (double) machine_width/SCREEN_WIDTH, (double) machine_height/SCREEN_HEIGHT );
	int w = SCREEN_WIDTH * scale, h = SCREEN_HEIGHT * scale;
    printf("x:%d y:%d w:%d h:%d\n",(machine_width - w )/2, (machine_height - h)/2, w, h);
    fill = (SDL_Rect){ (machine_width - w )/2, (machine_height - h)/2, w, h };
}
...
void guiDrawBitmap(uint16_t *bmp, int32_t x, int32_t y, int32_t w, int32_t h) {
    ...
    if (SDL_LockTexture(screenTexture, NULL , &pixels, &pitch) == 0) {
        ...
        SDL_RenderCopy(renderer, screenTexture, NULL, &fill);
...
int sdlkey2emu(int joy)
{
	switch(joy)
	{
		...
		case M_L2://注意这是自定义的摇杆的轴值，确认是否跟Button值重复
			return TRIGGERLEFT;
		case M_R2:
			return TRIGGERRIGHT;
	}
        ...
        while (isLoop && SDL_WaitEvent(&ev)){
            ...
            switch (ev.type) {
                case SDL_EVENT_TIMER:
                    ...
                    break;
                case SDL_KEYDOWN:
                    if (ev.key.keysym.sym == SDLK_ESCAPE || ev.key.keysym.sym == SDLK_HOME)
                    {
                        isLoop = false;
                    }
                    if (isKeyDown == SDLK_UNKNOWN) {
                        isKeyDown = ev.key.keysym.sym;
                        keyEvent(MR_KEY_PRESS, sdlkey2emu(ev.key.keysym.sym));
                    }
                    break;
                case SDL_KEYUP:
                    if (isKeyDown == ev.key.keysym.sym) {
                        isKeyDown = SDLK_UNKNOWN;
                        keyEvent(MR_KEY_RELEASE, sdlkey2emu(ev.key.keysym.sym));
                    }
                    break;
...
int main(int argc, char *argv[]) {
    ...
    getDestinationRect(machine_width, machine_height);
    ...
    window = SDL_CreateWindow(NULL, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, machine_width, machine_height,  SDL_WINDOW_SHOWN);
    if (window == NULL) {
    ...
    //SDL_RenderSetLogicalSize(renderer, SCREEN_WIDTH, SCREEN_HEIGHT);//miyoo mini的sdl2不支持
