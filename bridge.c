...
static void br__mr_c_function_new(BridgeMap *o, uc_engine *uc) {
    ...
    *(uint32_t *)getMrpMemPtr(CODE_ADDRESS + 4) = v;
...
static void br_mr_write(BridgeMap *o, uc_engine *uc) {
    ...
    ret = my_write(f, getMrpMemPtr(p), l);
...
static void br_mem_get(BridgeMap *o, uc_engine *uc) {
    ...
    *(uint32_t *)getMrpMemPtr(mem_base) = buffer;
    *(uint32_t *)getMrpMemPtr(mem_len) = len;
...
// 为原始PCM数据构造最小WAV头，使SDL_mixer可直接加载
static uint8_t *makePCMWav(const uint8_t *pcmData, uint32_t dataLen, uint32_t *outLen) {
    *outLen = dataLen + 44;  // 44字节WAV头
    uint8_t *wav = malloc(*outLen);
    if (!wav) return NULL;

    // RIFF头
    memcpy(wav + 0,  "RIFF", 4);
    uint32_t fileSize = *outLen - 8;
    memcpy(wav + 4,  &fileSize, 4);
    memcpy(wav + 8,  "WAVE", 4);

    // fmt chunk (16字节, PCM格式)
    memcpy(wav + 12, "fmt ", 4);
    uint32_t fmtSize = 16;
    ...
    uint32_t byteRate = 8000 * 1 * 2;  // sampleRate * channels * bitsPerSample/8
...
static Mix_Chunk* currentChunk = NULL;
...
static void br_mr_playSound(BridgeMap *o, uc_engine *uc) {
    ...
    return;
#else
    const uint8_t *soundData = (const uint8_t *)getMrpMemPtr(data);
    int loops = loop ? -1 : 0;

    switch (type) {
        case MR_SOUND_WAV: {
            SDL_RWops *rw = SDL_RWFromConstMem(soundData, dataLen);
            Mix_Chunk *chunk = Mix_LoadWAV_RW(rw, 1);
            if (chunk) {
                if (currentChunk) {
                    Mix_HaltChannel(0);
                    Mix_FreeChunk(currentChunk);
                    currentChunk = NULL;
                }
                if (Mix_PlayChannel(0, chunk, loops) < 0) {
                    printf("播放WAV失败: %s\n", Mix_GetError());
                    Mix_FreeChunk(chunk);
                } else {
                    currentChunk = chunk;
            ...
            SET_RET_V(chunk ? MR_SUCCESS : MR_FAILED);
            return;
        }
        case MR_SOUND_MIDI:
        case MR_SOUND_MP3: {
            ...
            SET_RET_V(music ? MR_SUCCESS : MR_FAILED);
            return;
        }
        case MR_SOUND_PCM: {
            uint32_t wavLen;
            uint8_t *wavBuf = makePCMWav(soundData, dataLen, &wavLen);
            if (!wavBuf) {
                SET_RET_V(MR_FAILED);
                return;
            }
            SDL_RWops *rw = SDL_RWFromConstMem(wavBuf, wavLen);
            Mix_Chunk *chunk = Mix_LoadWAV_RW(rw, 1);
            free(wavBuf);
            if (chunk) {
                if (currentChunk) {
...
#define BRIDGE_HASH_SIZE 1024
#define BRIDGE_HASH_MASK (BRIDGE_HASH_SIZE - 1)
...
static BridgeHashEntry bridgeHashTable[BRIDGE_HASH_SIZE];

static inline uint32_t bridge_hash_idx(uint32_t address) {
    return ((address >> 2) * 2654435761u) >> 24;
}

static BridgeMap *bridge_lookup(uint32_t address) {
    uint32_t idx = bridge_hash_idx(address);
    for (;;) {
        if (bridgeHashTable[idx].value == NULL) return NULL;
        if (bridgeHashTable[idx].key == address) return bridgeHashTable[idx].value;
        idx = (idx + 1) & BRIDGE_HASH_MASK;
    }
}

static int bridge_insert(uint32_t address, BridgeMap *value) {
    uint32_t idx = bridge_hash_idx(address);
    for (;;) {
        if (bridgeHashTable[idx].value == NULL) {
            bridgeHashTable[idx].key = address;
            bridgeHashTable[idx].value = value;
            return 0;
        }
        if (bridgeHashTable[idx].key == address) return -1;
        idx = (idx + 1) & BRIDGE_HASH_MASK;
    }
}
...
static void *hooks_init(uc_engine *uc, BridgeMap *map, uint32_t mapCount, uint32_t size) {
    ...
    for (int i = 0; i < mapCount; i++) {
        ...
        if (bridge_insert(addr, obj) != 0) {
            printf("bridge_insert() failed 0x%X exists.\n", addr);
            goto end;
        }
    }
    return ptr;
