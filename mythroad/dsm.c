...
static char *font_sky16_data = NULL;  // 预加载的字体数据缓冲区
static int font_sky16_data_len = 0;
...
static int xl_font_sky16_init() {  //字体初始化，打开字体文件
    ...
    // 获取字体文件大小并预加载到内存
    font_size = mr_getLen("system/gb16.uc2");
    if (font_size > 0) {
        font_sky16_data = (char *)mr_malloc(font_size);
        if (font_sky16_data) {
            if (mr_read(font_sky16_f, font_sky16_data, font_size) == font_size) {
                font_sky16_data_len = font_size;
                mr_close(font_sky16_f);
                font_sky16_f = 0;
                LOGI("font preload suc size=%d", font_size);
                return 0;
            }
            mr_free(font_sky16_data, font_size);
            font_sky16_data = NULL;
        }
    }

    // 回退：预加载失败则使用传统文件IO方式
    LOGI("font load suc fd=%d (fallback)", font_sky16_f);
    return 0;
}

static int xl_font_sky16_close() {  //关闭字体
    if (font_sky16_data) {
        mr_free(font_sky16_data, font_sky16_data_len);
        font_sky16_data = NULL;
        font_sky16_data_len = 0;
        return 0;
    }
...
static char *xl_font_sky16_getChar(uint16 id) {
    if (font_sky16_data && id * 32 + 32 <= font_sky16_data_len) {
        memcpy2(font_sky16_bitbuf, font_sky16_data + id * 32, 32);
...
static void xl_font_sky16_drawChar(uint16 ch, int x, int y, uint16 color) {
    ...
    if (font_sky16_data && ch * 32 + 32 <= font_sky16_data_len) {
        bitbuf = font_sky16_data + ch * 32;
    } else {
        mr_seek(font_sky16_f, ch * 32, 0);  // 一行两字节，高度16，所以2*16=32字节
        mr_read(font_sky16_f, font_sky16_bitbuf, 32);
        bitbuf = font_sky16_bitbuf;
    }
